...
export const updateUserDisplayName = async (displayName: string) => {
  ...
  // First, update the user's metadata with the new display name
  // This is the primary way the display name is stored in auth.users
  ...
  if (!userError) {
    try {
      // Call our SQL function to update the display name in auth.users table
      ...
      // Also update the user's profile in our profiles table
      ...
      // Force a refresh of the session to ensure updated metadata is available
      ...
      console.error('Error in display name update process:', error);
    }
  }
